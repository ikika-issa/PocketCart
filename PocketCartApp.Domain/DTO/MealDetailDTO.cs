...
namespace RecipeLibrary.Domain.DTO
{
    public class MealDetailDTO
    {
        ...
        // 🔹 Helper метод за clean iteration
        public List<(string Ingredient, string Measure)> GetIngredientsList()
        {
            var list = new List<(string Ingredient, string Measure)>();
            for (int i = 1; i <= 20; i++)
            {
                var ingredientObj = this.GetType().GetProperty($"StrIngredient{i}")?.GetValue(this);
                var measureObj = this.GetType().GetProperty($"StrMeasure{i}")?.GetValue(this);

                string? ingredient = ingredientObj?.ToString();
                string? measure = measureObj?.ToString();

                if (!string.IsNullOrWhiteSpace(ingredient))
                    list.Add((ingredient, measure ?? string.Empty));
            }
            return list;
        }
    }
